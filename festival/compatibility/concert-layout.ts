/** Canvas-space layout. The client applies display density; do not apply it twice. */
export function concertLayout(width: number, height: number, _dpr = 1, left = 0, right = 0, top = 0, bottom = 0, large = false) {
  const w = Math.max(0, width - left - right), h = Math.max(0, height - top - bottom)
  // SDK mobile guidance: devicePixelRatio does not participate in UI layout.
  const scale = large ? Math.max(1, Math.min(1.25, w / 360, h / 354)) : 1
