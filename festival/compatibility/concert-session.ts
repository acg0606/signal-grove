...
const LEASE = 8000
export const CHANNEL = 'affinity-arena-v9-concert'
...
export class ConcertSession {
  ...
  constructor(readonly id: string, now: number) {
    if ((!validId(id) || id.startsWith('bot:'))) throw new Error('Invalid player identity')
  ...
  private elect(now: number) {
    ...
    if (next !== this.host) {
      ...
      this.notice = changed ? 'Connection changed. Rejoin your crew; no winner awarded.' : 'Choose a studio. Rehearsal starts after the visible queue.'
  ...
  tick(now: number) {
    ...
    if (this.host === this.id) {
      for (const p of this.state.players) {
        if (!p.bot && !this.peers.has(p.id) && this.state.phase !== 'result') {
  ...
  receive(raw: unknown, sender: string, now: number) {
    if ((!validId(sender) || sender.startsWith('bot:')) || !raw || typeof raw !== 'object') return
    let size: number
    ...
    if (p.kind === 'state') {
      ...
      if (sender !== this.host) {
        if (this.state.phase !== 'lobby' || this.state.players.length !== 0 || !['training', 'intermission', 'presentation', 'battle', 'result'].includes(p.state.phase)) return
      ...
      this.pending = null
      this.notice = p.accepted ? 'Confirmed.' : 'Not accepted: crew full or note window closed.'
    }
  }
